...
def incremental_compute_gradient(grad,img,path):
    ...
    """
    # THIS CODE SHOULD BE REMOVED AND REPLACED TO FINISH THE EXERCISE:
    #compute_gradient(grad,img)
    # END OF CODE TO BE REMOVED
    #Añadidos
    width, height = len(grad[0]), len(grad)

    for y in (0, height-1): # just first and last rows
        for x in range(max(1,path[y]-2),min(width-1,path[y]+2)): # first and last columns are excluded
            grad[y][x] = abs(img[y][x-1] - img[y][x+1])
...
#completar
    ...
    for y in range(1,height):
        ...
        # COMPLETE
        for x in range(1,width-1):
            aux = min(mat[y-1][x-1],mat[y-1][x],mat[y-1][x+1])
            mat[y][x] = aux + grad[y][x]
    # in case of tie, retrieve the path with lowest (leftmost) column:
    min_val = infty
    min_point = -1
    ...
    for y in range(height-2, -1, -1):
        ...
        #Codigo mal
        #newmin = min( mat[y][x-1], mat[y][x], mat[y][x+1]);
        #if newmin == mat[y][x-1]:
        #    min_point = x-1;
        #else:
        #    if newmin == mat[y][x]:
