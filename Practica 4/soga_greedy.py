...
def greedy_tug_of_war(v):
    ...
    lista = v
    lista.sort()
    for indice in range(0,len(v)-1,2): #Esta fallando aqui, estoy metiendo en cada iteracion 0,1-1,2, cambia para que recorra la mitad del array
        accumleft,accumright = sum(left),sum(right)
        E1 = lista[indice]
        valido = False;
        if indice < len(v)-1:
            E2 = lista[indice+1]
            valido = True;
        if (abs((accumleft+E1)-(accumright+E2)) <= abs((accumleft+E2)-(accumright+E1))):
            left.append(E1)
            if valido:
                right.append(E2)
        else:
            ...
            if valido:
                left.append(E2)
    accumleft,accumright = sum(left),sum(right)
    print (N2,len(left), accumleft, len(right), accumright)
    return accumleft
